...
class Abril {
    ...
    public function editAbrilImprensa($conteudos,$clipings) {
        $sql = $this->pdo->prepare("SELECT * FROM abril");
        $sql->execute();
        if($sql->rowCount() > 0) {
    ...
    public function editAbrilDesign($pecas, $impressoes) {
        $sql = $this->pdo->prepare("SELECT * FROM abril");
        $sql->execute();
        if($sql->rowCount() > 0) {
    ...
    public function editAbrilFotografia($cobertura, $material) {
        $sql = $this->pdo->prepare("SELECT * FROM abril");
        $sql->execute();
        if($sql->rowCount() > 0) {
    ...
    public function editAbrilSocial($seguidores,$alcance) {
        $sql = $this->pdo->prepare("SELECT * FROM abril");
        $sql->execute();
        if($sql->rowCount() > 0) {
